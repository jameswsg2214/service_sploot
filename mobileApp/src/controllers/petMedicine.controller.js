...
const medicineController = () => {
        ...
        try {
            ...
            if (flag == 'update') {
                ...
                medicinedb.update(
                    ...
                    .then(() => {
                        return res.status(httpStatus.OK).json({
                            status: "sucess", msg: "updated sucessfully", req: postData
                        });
                    })
                    .catch(() => {
                        return res.status(httpStatus.ok).json({
                ...
                const petMedicine = medicinedb.create({
                    ...
                    .then(data => {
                        console.log(data)
                        res.json({ status: 'success', msg: 'inserted sucessfully', req: postData })
                    })
    ...
    const deleteMedicine = async (req, res, next) => {
        ...
        try {
            ...
            return res
                .status(httpStatus.OK)
                .json({ status: "sucess", req: postData, res: data });
        }
        catch (err) {
    ...
    const petMedicineBulk = async (req, res, next) => {
        ...
        if (petMedicinelist.length > 0) {
            try {
                ...
                return res.status(httpStatus.OK).json({ status: 'success', req: petMedicinelist, res: medicinedbImport });
            }
            catch (err) {
